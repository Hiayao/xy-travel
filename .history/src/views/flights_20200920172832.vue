      ...
      api
        ...
        .then((res: AirsRes) => {
          ...
          res.flights.map((item: any) => {
            ...
            item.arr_time = item.arr_time.split(":");
            item.dep_time = item.dep_time.split(":");
            item.arr_time.map((item1: any) => {
              item.dep_time.map((item2: any) => {
                if (item.dep_time[1] >= item.arr_time[1]) {
                  data.duration[1] = String(
                    Number(item.dep_time[1]) - Number(item.arr_time[1])
                  );
                  data.duration[0] = String(
                    Number(item.dep_time[0]) - Number(item.arr_time[0])
                  );
                }
                if (item.dep_time[1] < item.arr_time[1]) {
                  data.duration[1] = String(
                    Number(item.dep_time[1]) - Number(item.arr_time[1]) + 60
                  );
                  data.duration[0] = String(
                    Number(item.dep_time[0]) - Number(item.arr_time[0]) - 1
                  );
                }
              });
            });
