...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    const ChangeValue1 = (value: string): void => {
      console.log('selected',String(value)[0]+String(value)[7],String(value)[String(value).length-5]+String(value)[String(value).length-4],value.length);
      // data.flights = data.flights.filter((item: any) =>{
      //   return item.org_airport_name === value
      // })
  if(value.length ===11){
data.flights.map((item: any)=>{
      if(Number(String(value)[0])<Number(item.depTimes[0])&&Number(item.depTimes[7])<Number(String(value)[0])){
