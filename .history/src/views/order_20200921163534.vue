      ...
      <div class="BoxOne">
        ...
        <div class="Insurance">
          <div class="title1">保险</div>
					<div v-for="(item,index) in list.insurances" :key="index">
          <a-checkbox v-model:checked="checked" @change="onChange">{{item.type}}: ￥{{item.price}}/份x1 最高赔付{{item.compensation}}</a-checkbox>
        </div></div>
      </div>
