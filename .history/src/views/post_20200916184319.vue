  ...
  <div class="post_All flex jc">
    <div class="postBox flex jsb">
      ...
      <div class="postRight">
        ...
        <div v-for="(item,index) in postsList" :key="index" class="postsList">
          <div v-if="index%2===0">
            <div class="title">{{item.title}}</div>
            <div class="summary">{{item.summary}}</div>
            <div class="flex jsb" v-if="index <= 3">
          ...
          <div v-if="index%2===1" class="flex">
            <img :src="item.images[0]" alt class="imagesTwo" v-if="index <= 3"/>
  ...
  setup(props, ctx: SetupContext) {
    ...
    const onShowSizeChange = (current: number, pageSize: number): void => {
		...
		const onChange = (pageNumber: number): void => {
			...
			data.postsList = data.postsList.slice((data.pageNumber-1)*data.pageSize,pageNumber*data.pageSize)
		}
    onMounted(() => {
      // 城市菜单列表
      api
      ...
      api.getPostAll().then((res: PostRes) => {
        ...
        data.total = res.total;
			...
			data.postsList = data.postsList.slice((data.pageNumber-1)*data.pageSize,data.pageNumber*data.pageSize)
    });
    return {
      ...toRefs(data),
      fingerIn,
