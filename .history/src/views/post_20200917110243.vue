    ...
    <div class="postBox flex jsb">
          ...
          >
            ...
            <div class="hot poab" v-if="fleg === index">
              ...
              >
                ...
                <div class="cityName" @click="sousuo(item1)">{{item1.city}}</div>
                <div class="cityDesc" @click="sousuo(item1)">{{item1.desc}}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="postRight">
        <div class="pore">
          <input
            v-model="value"
            placeholder="输入想去的地方，比如：‘成都’"
            class="input"
            @keydown.enter="pressEnter"
          />
          <SearchOutlined class="SearchOutlined poab" @click="onSearch" />
        </div>
        <div class="proposal flex ac">
          <div>推荐：</div>
          <div v-for="(item,index) in proposal" :key="index" class="proposalItem" @click="again(item)">{{item}}</div>
        </div>
        ...
        <div
          v-for="(item,index) in postsList.slice((pageNumber-1)*pageSize,pageNumber*pageSize)"
          :key="index"
          class="postsList"
        >
          <div v-if="index%2===0">
            <div class="title" @click="goDetail(item)">{{item.title}}</div>
            <div class="summary" @click="goDetail(item)">{{item.summary}}</div>
            <div class="flex jsb">
              <div v-for="(item1,index1) in item.images.slice(0,3)" :key="index1" class="imgAll">
                <img :src="item1" alt class="images"  @click="goDetail(item)"/>
              </div>
            </div>
            <!-- <div v-if="index > 3"><img src="../assets/zanwu.jpg" class="images" alt=""></div> -->
          ...
          <div v-if="index%2===1" class="flex">
            <img :src="item.images[0]" alt class="imagesTwo"  @click="goDetail(item)"/>
            <!-- <img src="../assets/zanwu.jpg" class="imagesTwo" alt="" v-if="index > 3"> -->
            <div class="newsAll">
              <div class="title thidden-one"  @click="goDetail(item)">{{item.title}}</div>
              <div class="summary"  @click="goDetail(item)">{{item.summary}}</div>
              <div class="nameAll flex ac jsb">
                <EnvironmentOutlined />
                <div>{{item.city.name}}</div>
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    const getPost = (): void => {
      api
        ...
        .then((res: PostRes) => {
          ...
          data.postsList.map((item: any, index: number) => {
              if (index > 0) {
                item.images = [
                  "https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1600270480715&di=81d25a5f08bf87f2712a430d7846339a&imgtype=0&src=http%3A%2F%2Fimages.669pic.com%2Felement_pic%2F3%2F88%2F77%2F52%2F63160fa6edcfde1b73bafadf9be77e8d.jpg",
                ];
              }
            });
    ...
    const sousuo = (item1: any): void => {
      // console.log(item1);
      data.value = item1.city
      getPost()
    }
    const goCreate = (): void => {
    ...
    const goDetail = (item: any): void => {
      console.log(item);
      router.push({path:'/post/detail',query:{id:item.id}})
    }
    onMounted(() => {
      // 城市菜单列表
      api
