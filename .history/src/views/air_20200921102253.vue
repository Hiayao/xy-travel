...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    const change = (): void => {
      ...
      data.value = two;
      data.value2 = one;
      const ones = data.departCode;
      const twos = data.destCode;
      data.departCode = data.destCode;
      data.destCode = data.departCode;
    };
    ...
    const goAirLists = (): void => {
      if (data.departCode !== "" && data.destCode !== "") {
        if (data.value !== "" && data.value2 !== "" && data.departDate !== "") {
          router.push({
            path: "/air/flights",
            query: {
        ...
        message.warning("无此航行路线，请重新查询");
      }
    };
    onMounted(() => {
      proposalAirs();
      if (JSON.parse(localStorage.getItem("history") as string).length > 0) {
