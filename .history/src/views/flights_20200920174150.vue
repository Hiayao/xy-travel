      ...
      api
        ...
        .then((res: AirsRes) => {
          ...
          res.flights.map((item: any) => {
            ...
            item.arr_times = item.arr_time.split(":");
            item.dep_times = item.dep_time.split(":");
            item.arr_times.map((item1: any) => {
              item.dep_times.map((item2: any) => {
                if (Number(item.dep_times[1]) >= Number(item.arr_times[1])) {
                  data.duration[1] = String(
                    Number(item.dep_times[1]) - Number(item.arr_times[1])
                  );
                  data.duration[0] = String(
                    Number(item.dep_times[0]) - Number(item.arr_times[0])
                  );
                }
                if (Number(item.dep_times[1]) < Number(item.arr_times[1])) {
                  data.duration[1] = String(
                    Number(item.dep_times[1]) - Number(item.arr_times[1]) + 60
                  );
                  data.duration[0] = String(
                    Number(item.dep_times[0]) - Number(item.arr_times[0]) - 1
                  );
                }
              });
            });
            ...
            // }
            // if(data.time> 0 ){
            //   data.duration = String(data.time/60).split('.')
