    ...
    <div class="airBox">
      ...
      <div class="icoAll flex ac">
        ...
        <div class="proposal flex ac jsa">
          <div v-for="(item,index) in proposalList" :key="index" class="proposalItem flex ac pore" @click="goAirList(item)">
    ...
    const proposalAirs = (): void => {
      api
		...
		const goAirList = (item: any): void => {
			router.push({path:('/air/flights'),query:{departCity:item.departCity,departCode:item.departCode,destCity:item.destCity,destCode:item.destCode,departDate:item.departDate}})
		}
    onMounted(() => {
      proposalAirs();
    });
