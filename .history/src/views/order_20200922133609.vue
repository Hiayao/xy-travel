  ...
  setup(props, ctx: SetupContext) {
    ...
    const kong = (): void => {
      ...
      const flegsss = data.arr.every((item: any) => {
        return item.value3 === /^[1-9]\d{5}(?:18|19|20)\d{2}(?:0[1-9]|10|11|12)(?:0[1-9]|[1-2]\d|30|31)\d{3}[\dXx]$/;
      });
        ...
        .then(() => {
          ...
          data.arr.map((item: any) => {
            if (data.form.captchas !== data.code) {
              ...
              kong();
              if (data.err === false && data.errs === false && data.errss===false) {
                // console.log("好");
