...
export default {
    ...
    //提交机票订单
    airorders({ users, insurances, contactName, contactPhone, invoice, air }: { users: object[]; insurances: string[]; contactName: string; contactPhone: string; invoice: boolean; air: number }) {
        return service.post(`/airorders`, { users, insurances, contactName, contactPhone, invoice, air },
            {
                headers:
