...
export default {
    ...
    //提交机票订单
    airorders({ users, insurances, contactName, contactPhone, invoice,seat_xid, air ,captchas}: { users: object[]; insurances: string[]; contactName: string; contactPhone: string; invoice: boolean;seat_xid: string; air: number;captchas: number; }) {
        return service.post(`/airorders`, { users, insurances, contactName, contactPhone, invoice, seat_xid,air,captchas },
            {
                headers:
